...
#         read_len_list = []
    ...
    if config.sc_mode or config.st_mode:
        if config.barcode_in_readname:
            ...
            sep = config.barcode_separator
            parts = read_name.rsplit(sep, 2)
            if len(parts) == 3:
                cell_barcode = parts[1]
                umi = parts[2]
        else:
            # Read CB and UB from optional SAM fields (columns 12+)
            cb_prefix = config.cb_tag + ':Z:'
            ub_prefix = config.umi_tag + ':Z:'
            for field in fields[11:]:
                if field.startswith(cb_prefix):
                    cell_barcode = field[len(cb_prefix):].strip()
                elif field.startswith(ub_prefix):
                    umi = field[len(ub_prefix):].strip()
        # Strip 10x GEM group suffix (e.g. "ACGTACGT-1" → "ACGTACGT")
        if cell_barcode is not None:
            cell_barcode = re.sub(r'-\d+$', '', cell_barcode)

    return [read_name, read_start_pos, rname, read_len_list, cell_barcode, umi]
##########
...
def map_read(gene_points_dict,gene_interval_tree_dict,gene_regions_dict, 
    ...
    # mapping = {}
    read_name, read_start_pos, rname, read_len_list = parsed_line[0], parsed_line[1], parsed_line[2], parsed_line[3]
    tolerance = 20
