...
def construct_community(isoform_gene_dict,isoform_index_dict,output_path,threads):
    ...
    # Save labels for single-cell quantification
    np.save(f'{output_path}/temp/community/LR_labels.npy', LR_labels)
    np.save(f'{output_path}/temp/community/isoform_labels.npy', isoform_labels)
    return num_LRs
def EM_worker(worker_id,output_df,output_path,eff_len_arr,num_LRs):
    ...
    for community_cond_prob,community_isoform,community_id in zip(worker_file['cond_prob'],worker_file['isoform'],worker_file['community']):
        ...
        LR_expression_df = pd.DataFrame({'CPM':LR_expression,'theta':theta_arr},index=community_isoform)
...
def EM_manager(isoform_gene_dict,isoform_index_dict,eff_len_arr,output_df,output_path,threads):
    ...
    all_LR_TPM_df[['Isoform','Gene','CPM','theta','community','community_num_LRs','num_expected_LRs']].to_csv(f'{output_path}/LR_EM_expression.out',sep='\t',index=False)
    all_LR_TPM_df[['Isoform','Gene','CPM','num_expected_LRs']].sort_values(by=['Gene','Isoform']).to_csv(f'{output_path}/Isoform_abundance.out',sep='\t',index=False)
